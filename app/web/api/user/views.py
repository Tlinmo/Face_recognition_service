...
async def list_users(
    request: Request,
    offset: int = 0, limit: int = 10, session: AsyncSession = Depends(get_db_session), token: str = Depends(auth_header)
) -> List[IUser]:
...
async def user_info(id_: UUID, session: AsyncSession = Depends(get_db_session), token: str = Depends(auth_header)) -> IUser:
...
async def update(
    id_: UUID, _user: schema.UpdateUser, session: AsyncSession = Depends(get_db_session), token: str = Depends(auth_header)
):
